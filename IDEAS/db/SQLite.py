...
class SQLite:
    ...
    def insert(self, data=None, field=None, ignore=True, 
               header=False, errlog=None, reserved_words=[], **kwargs):
        """ 
        ...
        if not data:
            ...
            if header:
                field = data.next()
                field = [x for x in field if x!=""] # in case blank columns
                full_field = []
                for f in field:
                    if f in reserved_words:
                        full_field.append(f+"_")
                    else:
                        full_field.append(f)
                field = full_field
                ...
                try:
                    ...
                    for x in d:
                        if type(x).__name__ in ("str", "unicode"):
                            d_new.append(unicodedata.normalize('NFKD', unicode(x)).encode('ascii', 'ignore'))
                        else:
                            d_new.append(x)                            
                    self.c.execute(sql, d_new)
